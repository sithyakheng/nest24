...
function RankRequestContent() {
  ...
  const typeParam = searchParams.get('type') || 'forever';
  ...
  const prices = foreverPrices;
  const basePrice = prices[selectedTier];
  const finalPrice = basePrice;
  const currentQR = foreverQR[selectedTier];
      ...
      <div style={{ maxWidth: '620px', margin: '0 auto' }}>
        ...
        <div style={{ backgroundColor: 'white', border: isForever ? '2px solid #004E64' : '1px solid #e5e7eb', borderRadius: '24px', padding: '32px', textAlign: 'center', marginBottom: '36px', boxShadow: isForever ? '0 4px 24px rgba(0,78,100,0.15)' : '0 2px 12px rgba(0,0,0,0.06)' }}>
          ...
          <p style={{ color: '#6b7280', fontSize: '13px', marginBottom: '20px' }}>
            {tierNames[selectedTier]} Plan — $${foreverPrices[selectedTier]} one-time payment
          </p>
          <img
            key={currentQR}
            src={currentQR}
          ...
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', marginTop: '16px', background: 'rgba(0,78,100,0.07)', border: '1px solid rgba(0,78,100,0.25)', borderRadius: '9999px', padding: '6px 16px' }}>
            <span style={{ color: '#004E64', fontWeight: '800', fontSize: '18px' }}>${foreverPrices[selectedTier]}</span>
            <span style={{ color: '#6b7280', fontSize: '13px' }}>USD — {tierNames[selectedTier]} (one-time)</span>
          </div>
        ...
        <div style={{ backgroundColor: 'white', border: '1px solid #e5e7eb', borderRadius: '24px', padding: '32px', boxShadow: '0 2px 12px rgba(0,0,0,0.06)' }}>
          ...
          {/* No discount options for forever plans */}
          ...
          <div style={{ backgroundColor: '#f0f7fa', borderRadius: '14px', padding: '16px 20px', marginBottom: '24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <span style={{ fontWeight: '800', fontSize: '20px', color: '#004E64' }}>${finalPrice} (once)</span>
          </div>
